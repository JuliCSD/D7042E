...
@Component
public class WeatherSensorProviderWithPublishingApplicationInitListener extends ApplicationInitListener {
	...
	private final Logger logger = LogManager.getLogger(WeatherSensorProviderWithPublishingApplicationInitListener.class);
	...
	protected void customInit(final ContextRefreshedEvent event) {
		...
		getWeatherSensorServiceRequest.getMetadata().put(WeatherSensorProviderWithPublishingConstants.REQUEST_PARAM_KEY_TEMPERATURE, WeatherSensorProviderWithPublishingConstants.REQUEST_PARAM_TEMPERATURE);
		getWeatherSensorServiceRequest.getMetadata().put(WeatherSensorProviderWithPublishingConstants.REQUEST_PARAM_KEY_HUMIDITY, WeatherSensorProviderWithPublishingConstants.REQUEST_PARAM_HUMIDITY);
		getWeatherSensorServiceRequest.getMetadata().put(WeatherSensorProviderWithPublishingConstants.REQUEST_PARAM_KEY_PRESSURE, WeatherSensorProviderWithPublishingConstants.REQUEST_PARAM_PRESSURE);
		getWeatherSensorServiceRequest.getMetadata().put(WeatherSensorProviderWithPublishingConstants.REQUEST_PARAM_KEY_WIND, WeatherSensorProviderWithPublishingConstants.REQUEST_PARAM_WIND);
		arrowheadService.forceRegisterServiceToServiceRegistry(getWeatherSensorServiceRequest);
	...
	private ServiceRegistryRequestDTO createServiceRegistryRequest(final String serviceDefinition, final String serviceUri, final HttpMethod httpMethod) {
		...
		if (sslEnabled && tokenSecurityFilterEnabled) {
			...
			serviceRegistryRequest.setInterfaces(List.of(WeatherSensorProviderWithPublishingConstants.INTERFACE_SECURE));
		} else if (sslEnabled) {
			systemRequest.setAuthenticationInfo(Base64.getEncoder().encodeToString(arrowheadService.getMyPublicKey().getEncoded()));
			...
			serviceRegistryRequest.setInterfaces(List.of(WeatherSensorProviderWithPublishingConstants.INTERFACE_INSECURE));
		}
		serviceRegistryRequest.setProviderSystem(systemRequest);
		serviceRegistryRequest.setServiceUri(serviceUri);
		serviceRegistryRequest.setMetadata(new HashMap<>());
		serviceRegistryRequest.getMetadata().put(WeatherSensorProviderWithPublishingConstants.HTTP_METHOD, httpMethod.name());
