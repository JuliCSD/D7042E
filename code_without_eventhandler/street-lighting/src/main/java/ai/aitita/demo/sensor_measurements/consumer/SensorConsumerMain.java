package ai.aitita.demo.sensor_measurements.consumer;
...
public class SensorConsumerMain implements ApplicationRunner {
    ...
    private final Logger logger = LogManager.getLogger(SensorConsumerMain.class);
    ...
    public static void main( final String[] args ) {
    	SpringApplication.run(SensorConsumerMain.class, args);
    }

    //-------------------------------------------------------------------------------------------------
    @Override
	public void run(final ApplicationArguments args) throws Exception {
    	System.out.println("\n" + "Sensor Measurements Consumer has been started. Use this command line to trigger a new orchestration and consume the service.");    	
    	final Scanner sc = new Scanner(System.in);    	
    	commandLineUI(sc);    	
    	sc.close();
    	System.out.println("\n" + "Sensor Measurements Consumer has been terminated.");
    }
    ...
    private void commandLineUI(final Scanner sc) throws IOException, InterruptedException {
    	while (true) {   
    		try {
    			...
    			System.out.print("Hours to have measurements from now on (any positive int): ");
    			final int hours = Integer.parseInt(sc.nextLine());
    			
    			final OrchestrationResultDTO orchestrationResult = orchestrate(EFCommonConstants.SENSOR_MEASUREMENTS_SERVICE);
    			System.out.println("Orchestration result: ");
    			printOutJSON(orchestrationResult);
    			final SensorMeasurementsDTO sensorMeasurements = consumeSensorMeasurementsService(orchestrationResult, buildingId, LocalDateTime.now().plusHours(hours).toEpochSecond(ZoneOffset.UTC));
    			System.out.println("Service response: ");
    			printOutXML(sensorMeasurements);    			
