...
@Component
public class SensorMeasurementsDriver {
	...
	private final Logger logger = LogManager.getLogger(SensorMeasurementsDriver.class);
	
	//=================================================================================================
	// methods
	
	//-------------------------------------------------------------------------------------------------
	public SensorDetailsListDTO getLightSensorDetails(final long building, final long from, final long to) {
		final OrchestrationResultDTO orchestrationResult = orchestrate(EFCommonConstants.LIGHT_SENSOR_DETAILS_SERVICE);
		return consumeSensorDetailsService(orchestrationResult, building, from, to);		
	}
	
	//-------------------------------------------------------------------------------------------------
	public SensorDetailsListDTO getWeatherSensorDetails(final long building, final long from, final long to) {
		final OrchestrationResultDTO orchestrationResult = orchestrate(EFCommonConstants.WEATHER_SENSOR_DETAILS_SERVICE);
		return consumeSensorDetailsService(orchestrationResult, building, from, to);
	}
    ...
    //-------------------------------------------------------------------------------------------------
    private SensorDetailsListDTO consumeSensorDetailsService(final OrchestrationResultDTO orchestrationResult, final long building, final long from, final long to) {
		...
		return arrowheadService.consumeServiceHTTP(SensorDetailsListDTO.class, HttpMethod.valueOf(orchestrationResult.getMetadata().get(EFCommonConstants.HTTP_METHOD)),
